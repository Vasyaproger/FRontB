...
function LoginForm() {
  ...
  const handleSubmitLogin = async (e) => {
    e.preventDefault();
    try {
      const response = await fetch('vasyaproger-backentboodai-543a.twc1.net/api/login', {
        method: 'POST',
  ...
  const handleSubmitRegister = async (e) => {
    e.preventDefault();
    try {
      const response = await fetch('vasyaproger-backentboodai-543a.twc1.net/api/register', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ email, password }),
      });
